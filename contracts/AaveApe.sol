...
uint256 assetPrice = getPriceOracle().getAssetPrice(asset);
    ...
    uint256 assetAmount = (amountInEth * (10**decimals)) / (assetPrice);
return assetAmount;
...
return UNISWAP_ROUTER.swapExactTokensForTokens(amountOut, amountInMax, path, address(this), block.timestamp + 5);
...
function unwindApe(
    address apeAsset,
    address borrowAsset,
    uint256 interestRateMode) public {
...
function closePosition(address ape,
    address apeAsset,
    address borrowAsset,
    uint256 repayAmount,uint256 amountOwing,
    uint256 rateMode) internal returns(bool) {
...
IERC20(apeAsset).approve(UNISWAP_ROUTER_ADDRESS, maxCollateralAmount);
 ...
 // Deposit any leftover back into Aave on behalf of the user
    uint256 leftoverAmount  = maxCollateralAmount - (amounts[0]);
