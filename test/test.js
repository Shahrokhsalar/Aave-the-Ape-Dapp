...
const getLendingPool = async() => {
    ...
    let lendingPool = await getContractAt('ILendingPool', lendingPoolAddress)
...
const getAToken = async() =>{
    ...
    let assetAToken = await ethers.getContractAt('AToken',assetReserveData.aTokenAddress)
...
const getDebtToken = async(_asset, _interestRateMode, erc20=false) => {
    ...
    let interface = erc20===true?'IERC20' : 'interface '
...
describe ('describe', function() {
    ...
    beforeEach(async () => {
        const AaveApe  = await ethers.getContractAt("AaveApe");
        aaveApe = await AaveApe.deploy(lendingPoolAddressesProviderAddress,uniswapRouterAddress);


        const [user] = await ethers.getgetSigners();
    ...
    describe ("Address verification", function () {
        ...
        const _lendingPoolAddressesProvider  = await ethers.getContractAt('ILendingPoolAddressesProvider', lendingPoolAddressesProviderAddress)
            _lendingPoolAddressesProvider = await _lendingPoolAddressesProvider['getLendingPool']()
            expect(await aaveApe.LENDING_POOL()).to.equal(_lendingPoolAddresses);
    ...
    describe ("Going ape", function () {
        ...
        it ("Revert if the user has not delegated credit to the Ape",async function () {
            ...
            await depositEthInAave(userAddress, 2)
            await expect(aaveApe['ape'](wethAddress, daiAddress, 2).to.be.reverted);
        ...
        it("Succeeds if the user has collateral & has delegated credit",async function () {
            ...
            await delegateCreditToTheApe(delegateCreditToTheApe, interestRateMode)
            ...
            await expect(aaveApe['aoe'](wethAddress, daiAddress, interestRateMode).to.emit(aaveApe, "Ape"))
